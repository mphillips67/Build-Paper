...
names <- c('chr','pos','DBVPG6765','DBVPG6044','YPS128','Y12')

#K4
K4 <- read.table("4k_haps.txt", header= TRUE)
founders <- as.vector(str_split_fixed(K4[1,5], ";",4))
K4_freqs <- cbind(K4[,2:3],colsplit(K4$adjfounderfreqs, ";", founders))
for (i in 3:ncol(K4_freqs)){
  K4_freqs <- subset(K4_freqs, K4_freqs[i] >= 0 & K4_freqs[i] <= 1 )
}

K4_freqs$chr <- as.character(K4_freqs$chr)

replacement <- c('C10','C11','C12','C13','C14','C15','C16','C01','C02','C03','C04','C05','C06','C07','C08','C09') 

old <- as.vector(unique(K4_freqs$chr))
for (i in 1:16){
  
  K4_freqs[K4_freqs==old[i]] <- replacement[i]
}

names(K4_freqs) <- names

K4_freqs <- K4_freqs[,c(1,2,3,5,6,4)]
...
K4_freqs <- arrange(transform(K4_freqs,
                              chr=factor(chr,levels=chrs)),chr)

for (k in 1:length(chrs)){
  data.samp<-subset(K4_freqs,chr==chrs[k])
...
par(mfrow=c(4,1),mar=c(1,3,1,0),mgp=c(3,1,0), oma=c(1,0,1,0))
#plots
...
for (i in 3:ncol(K4_freqs)){
  ...
  plot(MB,K4_freqs[,i],
       ...
       main="",
       ylim=c(0,1),
       cex.main=2, #size of text for main title
       cex.lab=1, #size of text for axis labels
       type="n",	#type = none: don't make plot yet, need to make rectangles first
       axes=FALSE  #we will do the axes by hand next
  )
  title(ylab = colnames(K4_freqs[i]), cex.lab = 1.5,
        line = 1)
  ...
  points(MB,K4_freqs[,i],  type = "l", col = "blue" )
  
 
  #legend(11, 1.2, colnames(K4_freqs[i]),bty="n", cex=3)
  box() 
}
...
#S4
S4 <- read.table("4s_haps.txt", header= TRUE)
founders <- as.vector(str_split_fixed(S4[1,5], ";",4))
S4_freqs <- cbind(S4[,2:3],colsplit(S4$adjfounderfreqs, ";", founders))
for (i in 3:ncol(S4_freqs)){
  S4_freqs <- subset(S4_freqs, S4_freqs[i] >= 0 & S4_freqs[i] <= 1 )
}

S4_freqs$chr <- as.character(S4_freqs$chr)

replacement <- c('C10','C11','C12','C13','C14','C15','C16','C01','C02','C03','C04','C05','C06','C07','C08','C09') 
old <- as.vector(unique(S4_freqs$chr))
for (i in 1:16){
  
  S4_freqs[S4_freqs==old[i]] <- replacement[i]
}

names(S4_freqs) <- names

S4_freqs <- S4_freqs[,c(1,2,3,5,6,4)]
...
S4_freqs <- arrange(transform(S4_freqs,
                              chr=factor(chr,levels=chrs)),chr)

for (k in 1:length(chrs)){
  data.samp<-subset(S4_freqs,chr==chrs[k])
  if (chrs[k]=='C01'){Gaxis.samp<-data.samp$pos}
  if (chrs[k]=='C02'){Gaxis.samp<-data.samp$pos+230218}
  if (chrs[k]=='C03'){Gaxis.samp<-data.samp$pos+230218+813184}
  if (chrs[k]=='C04'){Gaxis.samp<-data.samp$pos+230218+813184+316620}
...
for (i in 3:ncol(S4_freqs)){
  ...
  title(ylab = colnames(S4_freqs[i]), cex.lab = 1.5,
        line = 1)
  #Here draw are gray rectangles that delineate chromosomes
  rect(0.23,-10,1.04,80,col="grey80",lty=0)  
  rect(1.36,-10,2.89,80,col="grey80",lty=0)  
  ...
  #Here draw points or lines.  Often it is useful to play around with point type, font size, color...
  points(MB,S4_freqs[,i],  type = "l", col = "blue" )
  #legend(11, 1.2, colnames(S4_freqs[i]),bty="n", cex=3)
  box() 
}
