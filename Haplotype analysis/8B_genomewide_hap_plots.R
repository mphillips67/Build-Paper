...
names <- c('chr','pos','DBVPG6765','DBVPG6044','BC187','L_1374','YPS128','Y12','L_1528','YJM981')

#K8
K8 <- read.table("8k_haps.txt", header= TRUE)
founders <- as.vector(str_split_fixed(K8[1,5], ";",8))
K8_freqs <- cbind(K8[,2:3],colsplit(K8$adjfounderfreqs, ";", founders))
for (i in 3:ncol(K8_freqs)){
  K8_freqs <- subset(K8_freqs, K8_freqs[i] >= 0 & K8_freqs[i] <= 1 )
}

K8_freqs$chr <- as.character(K8_freqs$chr)

replacement <- c('C10','C11','C12','C13','C14','C15','C16','C01','C02','C03','C04','C05','C06','C07','C08','C09') 
old <- as.vector(unique(K8_freqs$chr))
for (i in 1:16){
  
  K8_freqs[K8_freqs==old[i]] <- replacement[i]
}

names(K8_freqs) <- names

K8_freqs <- K8_freqs[,c(1,2,3,10,5,9,6,7,4,8)]
...
K8_freqs <- arrange(transform(K8_freqs,
                              chr=factor(chr,levels=chrs)),chr)

for (k in 1:length(chrs)){
  data.samp<-subset(K8_freqs,chr==chrs[k])
...
par(mfrow=c(8,1),mar=c(1,3,1,0),mgp=c(3,1,0), oma=c(1,0,1,0))
#plots
...
for (i in 3:ncol(K8_freqs)){
  ...
  title(ylab = colnames(K8_freqs[i]), cex.lab = 1.5,
        line = 1)
  ...
  points(MB,K8_freqs[,i],  type = "l", col = "blue" )
  #legend(11.1, 2, colnames(S8_freqs[i]),bty="n", cex=3)
  box() 
}

#add chromosome labels at midpts
mtext("C2",line = .7,side=1, at =0.635, cex=1.5)
mtext("C4",line = .7,side=1, at =2.125, cex=1.5)
mtext("C6",line = .7,side=1, at =3.595, cex=1.5)
mtext("C8",line = .7,side=1, at =5.105, cex=1.5)
mtext("C10",line = .7,side=1, at =6.2, cex=1.5)
mtext("C12",line = .7,side=1, at =7.78, cex=1.5)
mtext("C14",line = .7,side=1, at =9.635, cex=1.5)
mtext("C16",line = .7,side=1, at =11.595, cex=1.5)
...
#S8
S8 <- read.table("8S_haps.txt", header= TRUE)
founders <- as.vector(str_split_fixed(S8[1,5], ";",8))
S8_freqs <- cbind(S8[,2:3],colsplit(S8$adjfounderfreqs, ";", founders))
for (i in 3:ncol(S8_freqs)){
  S8_freqs <- subset(S8_freqs, S8_freqs[i] >= 0 & S8_freqs[i] <= 1 )
}

S8_freqs$chr <- as.character(S8_freqs$chr)

replacement <- c('C10','C11','C12','C13','C14','C15','C16','C01','C02','C03','C04','C05','C06','C07','C08','C09') 
old <- as.vector(unique(S8_freqs$chr))
for (i in 1:16){
  
  S8_freqs[S8_freqs==old[i]] <- replacement[i]
}

names(S8_freqs) <- names

S8_freqs <- S8_freqs[,c(1,2,3,10,5,9,6,7,4,8)]
...
S8_freqs <- arrange(transform(S8_freqs,
                              chr=factor(chr,levels=chrs)),chr)

for (k in 1:length(chrs)){
  data.samp<-subset(S8_freqs,chr==chrs[k])
  if (chrs[k]=='C01'){Gaxis.samp<-data.samp$pos}
  if (chrs[k]=='C02'){Gaxis.samp<-data.samp$pos+230218}
  if (chrs[k]=='C03'){Gaxis.samp<-data.samp$pos+230218+813184}
  if (chrs[k]=='C04'){Gaxis.samp<-data.samp$pos+230218+813184+316620}
...
for (i in 3:ncol(S8_freqs)){
  ...
  title(ylab = colnames(S8_freqs[i]), cex.lab = 1.5,
        line = 1)
  ...
  #Here draw points or lines.  Often it is useful to play around with point type, font size, color...
  points(MB,S8_freqs[,i],  type = "l", col = "blue" )
  legend(11.1, 2, colnames(S8_freqs[i]),bty="n", cex=3)
  box() 
}
